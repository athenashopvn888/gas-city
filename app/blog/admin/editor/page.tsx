...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("GAS CITY CANNABIS Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=GCC01&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "GAS CITY CANNABIS Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "GCC01" };
