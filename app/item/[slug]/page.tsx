...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | GAS CITY CANNABIS GTA`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://www.gascitycannabis.com/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | GAS CITY CANNABIS`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: Record<string, unknown> = {
    "@type": "Offer",
    url: `https://www.gascitycannabis.com/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://www.gascitycannabis.com${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://www.gascitycannabis.com/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://www.gascitycannabis.com/item/${item.slug}`
      }
    ]
  };
}
